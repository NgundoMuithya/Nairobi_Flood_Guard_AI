...
aggregates to ward level via a spatial grid, and computes the three rainfall features
...
Open-Meteo rate-limits by weighted call count (locations × days/14 × variables/10).
...
DEFAULT_GRID_RES = 0.25  # degrees (~28 km; fewer API calls than 0.1°)
...
def _compute_features(daily_mm: list[float]) -> dict[str, float]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 429:
                # Transient: rate-limited, worth waiting and retrying.
            ...
            if 400 <= resp.status_code < 500:
                # Permanent: bad params, malformed coords, etc. Retrying won't
                # help and previously burned minutes per batch on this.
                raise RuntimeError(
                    f"Open-Meteo rejected request (HTTP {resp.status_code}): "
                    f"{resp.text[:300]}"
                )

            resp.raise_for_status()  # transient 5xx -> falls through to retry below
            data = resp.json()
...
def fetch_grid_rainfall(
    ...
    features: dict[tuple[float, float], dict[str, float]] = {}
    ...
    for batch_idx, i in enumerate(range(0, len(grid_points), BATCH_SIZE)):
        ...
        responses = _fetch_batch(lats, lons, past_days=past_days)

        for point, payload in zip(batch, responses):
            daily = payload.get("daily", {})
            precip = daily.get("precipitation_sum", [])
            features[point] = _compute_features(precip)
...
def _save_file_cache(
    grid_features: dict[tuple[float, float], dict[str, float]],
    meta: dict[str, Any],
) -> None:
    try:
        ...
        # filesystem or permissions issue here shouldn't fail the fetch
        # that just succeeded.
        pass
...
def apply_live_rainfall(
    ...
    """
    Replace static rainfall columns with live Open-Meteo values.
    ...
    if use_cache:
        ...
        if cached is not None and cached.get("grid_resolution_deg") == resolution:
    ...
    df = _assign_rainfall(df, grid_features)
    return df, meta
...
def rainfall_summary(meta: dict[str, Any]) -> str:
    ...
    if meta.get("stale_cache"):
        return f"Live · Open-Meteo · stale cache from {ts_label}{scope_note}"
    cache_note = " (cached)" if meta.get("from_cache") else ""
    return f"Live · Open-Meteo · updated {ts_label}{cache_note}{scope_note}"
